  ...
  //    timer(new QTimer)
{
    ...
    connect(m_pWidget,SIGNAL(displayGraph(qint32,QString,QColor)),this,SLOT(display_graph(int)));//显示曲线
    connect(m_pWidget,SIGNAL(change_background(int)),this,SLOT(change_backgroundColor(int)));//改变表格背景颜色
    connect(m_pWidget,SIGNAL(change_graphColor(qint32,QString,QColor)),this,SLOT(setGraphColor(qint32,QString,QColor)));//设置曲线颜色

    connect(m_pWidget,SIGNAL(removeGraph(int)),this,SLOT(remove_graph(int)));//隐藏曲线
    ...
    //移动表格
    connect(m_pWidget,SIGNAL(up()),this,SLOT(chartUp()));
    connect(m_pWidget,SIGNAL(down()),this,SLOT(chartDown()));
    connect(m_pWidget,SIGNAL(left()),this,SLOT(chartLeft()));
    connect(m_pWidget,SIGNAL(right()),this,SLOT(chartRight()));

    connect(m_pWidget,SIGNAL(MarkingPork(bool)),this,SLOT(markingPoint(bool)));

    connect(m_pWidget,SIGNAL(horizontalGrid(bool)),this,SLOT(horizontalGridVisible(bool)));
    connect(m_pWidget,SIGNAL(verticalGrid(bool)),this,SLOT(verticalGridVisible(bool)));
}
...
void ShowChart_module_Widget::Chart_init()
{
    ...
    ui->chart_plot->xAxis->setNumberFormat("gbc");//设置x轴格式:g灵活的格式,b漂亮的指数形式，c乘号改成×
    //     ui->chart_plot->xAxis->setNumberPrecision(0.1);//精度0.1
    ...
    //设置图例位置
    //    ui->chart_plot->axisRect()->insetLayout()->setInsetAlignment(6,Qt::AlignLeft|Qt::AlignTop);
    //设置曲线可拖拽 滚轮放大缩小 图像可选择
    //    ui->chart_plot->setInteractions(QCP::iRangeDrag | QCP::iRangeZoom | QCP::iSelectPlottables);
    ui->chart_plot->setInteractions(QCP::iRangeZoom | QCP::iSelectPlottables|QCP::iSelectAxes);
...
void ShowChart_module_Widget::addNewGraph(qint32 currentNum,QString graph_name,QColor graph_color)
{
    ...
    qDebug()<<"添加曲线"<<currentNum<<endl;

    //    ui->chart_plot->graph(currentNum)->cp
...
//设置曲线颜色
void ShowChart_module_Widget::setGraphColor(qint32 index,QString graph_name,QColor graph_color)
{
    ...
    ui->chart_plot->graph(index)->setPen(pen); // line color blue for first graph
    ui->chart_plot->graph(index)->setName(graph_name);//默认曲线名字
    ui->chart_plot->graph(index)->setSelectable(QCP::stNone);// 禁止鼠标选择曲线
    ui->chart_plot->replot();
...
/**
 * @brief ShowChart_module_Widget::chartUp
 * 向上移动图
 */
void ShowChart_module_Widget::chartUp()
{
    ui->chart_plot->yAxis->moveRange(50);
    ui->chart_plot->replot(QCustomPlot::rpQueuedReplot);
}

void ShowChart_module_Widget::chartDown()
{
    ui->chart_plot->yAxis->moveRange(-50);
    ui->chart_plot->replot(QCustomPlot::rpQueuedReplot);
}

void ShowChart_module_Widget::chartLeft()
{
    ui->chart_plot->xAxis->moveRange(-50);
    ui->chart_plot->replot(QCustomPlot::rpQueuedReplot);
}

void ShowChart_module_Widget::chartRight()
{
    ui->chart_plot->xAxis->moveRange(50);
    ui->chart_plot->replot(QCustomPlot::rpQueuedReplot);
}
...
/**
 * @brief ShowChart_module_Widget::actionselectZoom
 * @param c
 * 设置框选放大
 */
...
/**
 * @brief ShowChart_module_Widget::markingPoint标记点
 * @param checked
 */
void ShowChart_module_Widget::markingPoint(bool checked)
{
    if(checked)
    {
        for (int i = 0;i<numGraphs;i++) {
            ui->chart_plot->graph(i)->setLineStyle((QCPGraph::LineStyle::lsNone));
            ui->chart_plot->graph(i)->setScatterStyle(QCPScatterStyle(QCPScatterStyle::ScatterShape::ssDisc,5));
        }
    }
    else
    {
        for (int i = 0;i<numGraphs;i++) {
            ui->chart_plot->graph(i)->setLineStyle((QCPGraph::LineStyle::lsLine));
            ui->chart_plot->graph(i)->setScatterStyle(QCPScatterStyle(QCPScatterStyle::ScatterShape::ssNone,2));
        }
    }
...
/**
 * @brief ShowChart_module_Widget::horizontalGridVisible
 * @param checked 网格设置
 */
void ShowChart_module_Widget::horizontalGridVisible(bool checked)
{
    ui->chart_plot->yAxis->grid()->setVisible(checked);
    ui->chart_plot->replot(QCustomPlot::rpQueuedReplot);
}
void ShowChart_module_Widget::verticalGridVisible(bool checked)
{
    ui->chart_plot->xAxis->grid()->setVisible(checked);
    ui->chart_plot->replot(QCustomPlot::rpQueuedReplot);
